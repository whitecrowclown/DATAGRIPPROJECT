---
title: "베이스 구문"
source: "https://obsidian.md/ko/help/bases/syntax"
author:
published:
created: 2026-04-23
description: "슬라이드 - Obsidian 도움말"
tags:
  - "clippings"
---
Obsidian에서 [베이스를 생성](https://obsidian.md/ko/help/bases/create-base) 하면 `.base` 파일로 저장돼요. 베이스는 일반적으로 앱 인터페이스를 사용하여 편집하지만, 구문을 수동으로 편집하거나 코드 블록에 임베드할 수도 있어요.

[베이스](https://obsidian.md/ko/help/bases) 구문은 [보기](https://obsidian.md/ko/help/bases/views), 필터 및 [수식](https://obsidian.md/ko/help/formulas) 을 정의해요. 베이스는 아래에 정의된 스키마를 따르는 유효한 YAML이어야 해요.
...
- 함수. 다양한 [함수](https://obsidian.md/ko/help/bases/functions) 가 내장되어 있으며, 플러그인이 추가 함수를 제공할 수 있어요.

필터와 수식의 구문 및 사용 가능한 함수는 동일해요.

### 수식

`formulas` 섹션은 베이스 파일의 모든 뷰에서 표시할 수 있는 [수식 속성](https://obsidian.md/ko/help/formulas) 을 정의해요.
...
수식 속성은 기본 산술 연산자와 다양한 내장 [함수](https://obsidian.md/ko/help/bases/functions) 를 지원해요. 향후 플러그인이 수식에서 사용할 함수를 추가할 수 있게 될 거예요.
...
[보기](https://obsidian.md/ko/help/bases/views) 는 상태를 유지하거나 적절히 렌더링하는 데 필요한 모든 정보를 저장하기 위해 추가 데이터를 추가할 수 있지만, 플러그인 작성자는 코어 베이스 플러그인에서 이미 사용 중인 키를 사용하지 않도록 주의해야 해요. 예를 들어, 표 뷰는 이를 사용하여 행 수를 제한하거나 정렬에 사용할 열과 방향을 선택할 수 있어요. 지도와 같은 다른 뷰 유형은 노트의 어떤 속성이 위도와 경도에 해당하는지, 어떤 속성이 핀 제목으로 표시되어야 하는지를 매핑하는 데 사용할 수 있어요.
...
[노트 속성](https://obsidian.md/ko/help/properties) 은 Markdown 파일에서만 사용할 수 있으며, 각 노트의 YAML 프런트매터에 저장돼요. 이러한 속성은 `note.author` 형식 또는 간단히 `author` 와 같은 축약형으로 접근할 수 있어요.

### 파일 속성

파일 속성은 현재 테스트 또는 평가 중인 파일을 나타내요. 파일 속성은 첨부 파일을 포함한 모든 [파일 유형](https://obsidian.md/ko/help/file-formats) 에서 사용할 수 있어요.
...
베이스가 다른 파일에 임베드되면, `this` 는 *임베딩하는* 파일(베이스를 포함하는 노트 또는 Canvas)의 속성을 가리켜요. 예를 들어, `this.file.name` 을 사용하면 베이스가 아닌 임베딩 파일의 이름을 반환해요.
...
전역 [함수](https://obsidian.md/ko/help/bases/functions) `today()` 를 사용하여 현재 날짜를 가져올 수 있고, `now()` 를 사용하여 시간이 포함된 현재 날짜를 가져올 수 있어요.
...
수식과 [필터](https://obsidian.md/ko/help/bases/views) 에서 사용할 수 있는 [함수 목록](https://obsidian.md/ko/help/bases/functions) 을 참조하세요.
...
날짜는 생성에 사용된 함수 또는 [속성](https://obsidian.md/ko/help/properties) 에 할당된 유형에 따라 특정 날짜 또는 날짜와 시간을 나타내요.
...
- 날짜 객체에서 사용할 수 있는 다양한 [필드와 함수](https://obsidian.md/ko/help/bases/functions) 가 있어요.
...
[위키 링크](https://obsidian.md/ko/help/link-notes) 가 [프런트매터 속성](https://obsidian.md/ko/help/properties) 에 있으면 자동으로 링크 객체로 인식돼요. 링크는 [뷰](https://obsidian.md/ko/help/bases/views) 에서 클릭 가능한 링크로 렌더링돼요.

- 링크를 생성하려면 전역 `link` [함수](https://obsidian.md/ko/help/bases/functions) 를 사용해요. 예: `link("filename")` 또는 `link("https://obsidian.md")`.
